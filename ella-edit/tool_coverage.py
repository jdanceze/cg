...
def coverage_for_apk(apk_name, coverage_type):
	ella_run_dir = "/workspace/ella-master6/" + utils.apk_name_to_ellaout_dir(apk_name)
	...
	if coverage_type == "method":
		...
		#write out get_current_coverage to a file
		with open(os.path.join(ella_run_dir,"method_coverage"),"w") as covf:
			covf.write(str(cov.get_current_coverage()))
	elif coverage_type == "activity":
